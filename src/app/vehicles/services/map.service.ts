...
export class MapService {
  ...
  timeWait: number = 720000; // 7200 segundos en milisegundos
  ...
  monitor(data: any, map: any): void {
    if (this.vehicleService.statusDataVehicle) {
      ...
      /* console.log(
        'ESTO ES ANTES DE VEHICLES CONDICIONAL',
        this.vehicleService.statusDataVehicle
      ); */
      /* console.log('ESTO ES VEHICLES', vehicles);

      console.log('INFO VEHICLE--> ,', this.vehicleService); */
      ...
      if (resultado) {
        ...
        /* console.log('POSICION DEL IMEI POR INDEX ', index); */
        ...
        if (this.imeiPopup == data.IMEI.toString()) {
          ...
          console.log('DATOS DEL POPUP DE LA CONDICION1', this.imeiPopup);
          console.log('DATOS DEL POPUP DE LA CONDICION2', data.IMEI.toString());
        }
        ...
        if (vehicles[index].eye) {
          ...
          for (const key in object) {
            ...
            if (
              ...
              /* console.log('INFO VEHICLE--> ,', vehicles[index].icon_color); */

              console.log('TRAMA', data.IMEI, data.data1);
              ...
              if (
                ...
                /*  const colorHex = '80CF52'; // Usar el valor de icon_color o un valor predeterminado
                  const svgPath = `../../../assets/images/objects/nuevo/bus/${vehicles[index].icon_name}.svg`; */
                ...
                if (
                  ...
                  if (
                    ...
                    // VERDE
                    iconUrl =
                      './assets/images/objects/nuevo/bus/bus_green.svg'; /* +
                      vehicles[index].icon_def; */

                    /* const colorHex = '80CF52'; // Usar el valor de icon_color o un valor predeterminado
                    const svgPath = `../../../assets/images/objects/nuevo/gif/pruebasvg/${vehicles[index].icon_name}.svg`; */
                    ...
                    // ROJO
                    iconUrl =
                      './assets/images/objects/nuevo/bus/bus_red.svg'; /* +
                      vehicles[index].icon_def;  */

                    /* const colorHex = 'FA0000'; // Usar el valor de icon_color o un valor predeterminado
                    const svgPath = `../../../assets/images/objects/nuevo/gif/pruebasvg/${vehicles[index].icon_name}.svg`; */
                    ...
                    // AZUL
                    iconUrl =
                      './assets/images/objects/nuevo/bus/bus_blue.svg'; /* +
                      vehicles[index].icon_def; */

                    /* const colorHex = '0000FF'; // Usar el valor de icon_color o un valor predeterminado
                    const svgPath = `../../../assets/images/objects/nuevo/gif/pruebasvg/${vehicles[index].icon_name}.svg`; */
                  ...
                  if (
                    ...
                    if (vehicles[index].icon_name != null) {
                      const colorHex = '80CF52';
                      const svgPath = `../../../assets/images/objects/nuevo/gif/pruebassvg/${vehicles[index].icon_name}.svg`;
                      ...
                      fetch(svgPath)
                        ...
                        .then((svgText) => {
                          const updatedSvgText = svgText.replace(
                            /<svg(.*?)>/,
                            `<svg$1 style="fill: #${colorHex}">`
                          );

                          const div = document.createElement('div');
                          div.innerHTML = updatedSvgText.trim();

                          const svgElement = div.querySelector(
                            'svg'
                          ) as SVGElement;
                          if (svgElement) {
                            const updatedIconUrl = `data:image/svg+xml;base64,${btoa(
                              div.innerHTML
                            )}`;
                            this.markerClusterGroup.getLayers()[key]['options'][
                              'icon'
                            ]['options']['iconUrl'] = updatedIconUrl;
                          }
                        })
                        .catch((error) =>
                    ...
                    // ROJO
                    /* iconUrl = '../../../assets/images/objects/nuevo/gif/onda_roja.gif' */
                    iconUrl =
                ...
                /* VehicleService.busSVG(
                  svgPath,
                  colorHex,
                  (updatedIconUrl: string) => {
                    this.markerClusterGroup.getLayers()[key]['options']['icon'][
                      'options'
  ...
  private drawIcon(data: any, map: any): void {
    ...
    /* const colorHex = '80CF52'; // Usar el valor de icon_color o un valor predeterminado
    const svgPath = `../../../assets/images/objects/nuevo/bus/bus.svg`; */

    
     
    // let iconOndaUrl = './assets/images/user_config/circulo_movimiento.svg';

    /* this.userDataService.busSVGCallback().then((urlFicticia) => {
      console.log('OBTENER DATOS', urlFicticia);
    }); */

   /*  const svgPath = './assets/images/objects/nuevo/bus/bus.svg';
    let colorHex = data.icon_color; */
    ...
    /* this.userDataService.busSVGCallback().then((urlFicticia) => {
      ...
      // Continuar con el código usando urlFicticia según sea necesario.
    /* }).catch((error) => {
      console.error('Error al obtener la URL ficticia:', error);
      // Manejar el error si es necesario.
    }); */
    ...
    /* userDataService.busSVGCallback(
      svgPath,
      colorHex,
      (bussvg: string) => { */
        // Dentro de este callback, puedes crear el objeto iconMarker y utilizar el updatedIconUrl
        ...
        // Ahora iconMarker está disponible en este ámbito
      /* },
      (error: Error) => {
        console.error('Error al actualizar el SVG:', error);
        // Manejar el error si es necesario
