...
export class MeasureDrawService {
  ...
  constructor( public mapService: MapServicesService) {
      ...
      this.map = this.mapService.map ;//options.map;
      //@ts-ignore
      this.layerGroup = L.layerGroup();
      this.layerGroup.addTo(this.map);
      this.measure = false;//options.measure ? options.measure : true
  }
  ...
  private _toolTip(latlng: L.LatLng) {
      ...
      if (latlng) {
          ...
          this.markerTip.bindTooltip("Click derecho para terminar", { className: 'draw-leaflet-tip', offset: [0, 20] }).openTooltip();
      }
  }
  //创建线
  createLine() {
